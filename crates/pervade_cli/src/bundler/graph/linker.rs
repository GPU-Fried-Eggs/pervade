...
use crate::bundler::module::{Module, Resolution};
use crate::bundler::symbols::Symbols;
use crate::common::{ModuleId, ResolvedExport, SymbolRef};
    ...
    fn mark_whether_namespace_referenced(graph: &mut Graph) {
        for id in &graph.sorted_modules {
            ...
            let importee_list = importer
                ...
                .iter()
                .filter_map(|rec| {
                    (rec.is_import_namespace && rec.resolved_module.is_valid())
            ...
            Module::Normal(importer) => {
                ...
                fn create_local_symbol_and_reference(
                    ...
                    let local_symbol = if symbol_ref.owner == exporter {
                        ...
                        symbols.tables[exporter].create_symbol(CompactStr::new("#FACADE#"))
                    };
                ...
                importer.named_exports.keys().for_each(|exported| {
                    ...
                    match res {
                        ...
                        Resolution::Found(ext) => {
                            let tmp = create_local_symbol_and_reference(
                                ext,
                                importer.id,
                                &mut self.graph.symbols,
                            );
